...
def DoRun(path, payload_type, payload_file_name, cover_file_name, path_payload_, path_cover_):
    ...
    folder_logs = os.path.join(path,'data_hasil_strt_pt','fga',payloadTypeFolder,'logs')
    hasil_logs_file = 'fga_with-sp_payload_0-3_0-3_1000_20.csv'
    ...
    path_hasil = os.path.join(path,'data_hasil_strt_pt','fga',payloadTypeFolder,hasil_logs_file)
    ...
    if(payloadType == 'image'):
        ...
        # path_save_grey = os.path.join(path, 'data_hasil_strt_pt', 'fga','payload-img','payload','grey_st_pt_payload-img_'+file_payload)
    ...
    path_save_grey_cover = os.path.join(path, 'data_hasil_strt_pt', 'fga',payloadTypeFolder,'cover',"".join(file_payload.split(".")[:-1])+"_"+file_cover)
    ld.save_img(coverImage,path_save_grey_cover)
    coverBin = ld.img_to_bin(coverImage)
    obFga = fga(0.3,0.3,1000,20,coverBin,twoDPayload, payloadShape, payloadT, os.path.join(folder_logs,"".join(file_payload.split(".")[:-1])+'-_-'+"".join(file_cover.split(".")[:-1])+'.csv'))
    newBinImg, bestAt, best = obFga.Run()
    path_save_stego = os.path.join(path, 'data_hasil_strt_pt', 'fga',payloadTypeFolder,'stego','st_pt_stgo_'+"".join(file_payload.split(".")[:-1])+"_"+file_cover)
    ...
    lg.writeResult(path_hasil,
        file_cover, file_payload, path_cover, path_payload, psnr, payloadType, payloadT, 'fga srtr pt + with chr fix bubble', bestAt, best_str,path_save_stego)
