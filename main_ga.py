from lib_ga.ga import Ga as ga
import lib_ga.stegonize as steg
import lib_ga.load_data as ld
import os
import lib_ga.logger as lg
import glob

def DoRun(path, payload_type, payload_file_name, cover_file_name, path_payload_, path_cover_):
    payloadTypeFolder = 'payload-text'

    folder_logs = os.path.join(path,'data_hasil','ga',payloadTypeFolder,'logs')
    ...
    path_hasil = os.path.join(path,'data_hasil','ga',payloadTypeFolder,hasil_logs_file)
    ...
    path_save_grey_cover = os.path.join(path, 'data_hasil', 'ga',payloadTypeFolder,'cover',"".join(file_payload.split(".")[:-1])+"_"+file_cover)
    ...
    path_save_stego = os.path.join(path, 'data_hasil', 'ga',payloadTypeFolder,'stego','stgo_'+"".join(file_payload.split(".")[:-1])+"_"+file_cover)
    ...
    lg.writeResult(path_hasil,
        file_cover, file_payload, path_cover, path_payload, psnr, payloadType, payloadT, 'ga fitness with chr fix bubble', bestAt, best_str,path_save_stego)
